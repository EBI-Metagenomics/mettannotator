...
Other parameters
  ...
  --extend_to_full                   [string]  Path to a previous --fast output directory. Loads existing results and
                                               runs only InterProScan, SanntiS and UniFIRE. Cannot be used with --fast.
  --allow_missing_files              [boolean] When used with --extend_to_full, downgrades missing optional fast-run
                                               tool outputs (CRISPR, AMR, BGC tools, DefenseFinder) from hard errors
                                               to warnings.
  --skip_fast_staging                [boolean] When used with --extend_to_full, skips copying fast-run outputs into
                                               --outdir. Use when you only want extended-tool results in the output
                                               directory.
  --ignore_version_mismatch          [boolean] Downgrades the pipeline version mismatch from a hard error to a
                                               warning. By default, running --extend_to_full against outputs from
...
It cannot be combined with `--fast`, `--extend_to_full`, or the samplesheet `gene_calls_gff` column.
...
### Two-phase annotation (--extend_to_full)

If you have already run the pipeline in `--fast` mode and want to add InterProScan, SanntiS and UniFIRE annotations
without re-running the full pipeline, use the `--extend_to_full` flag pointing to the previous fast-run output directory:
...
nextflow run ebi-metagenomics/mettannotator \
   ...
   --outdir <OUTDIR> \
   --dbs <PATH/TO/DBS> \
   --extend_to_full <PATH/TO/FAST/OUTDIR>
```
...
By default, the fast-run outputs are also staged into `--outdir` so the final directory contains both fast and extended-tool
results. To write only the extended-tool results, add `--skip_fast_staging`.
...
downgrade these to warnings and allow the full run to proceed:
...
| --------------------- | -------------------------------------------------------------------------- | --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ncRNA, rRNA           | all\*                                                                      | cmscan + Rfam   | ncRNA annotation (excluding tRNA)                                                                                                                                                                           |
...
| CDS                   | `antismash_gene_function`                                                  | antiSMASH       | BGC gene function                                                                                                                                                                                           |
| CDS                   | `antismash_product`                                                        | antiSMASH       | BGC product                                                                                                                                                                                                 |
| CDS                   | `amrfinderplus_element_symbol`                                             | AMRFinderPlus   | Gene symbol according to AMRFinderPlus                                                                                                                                                                      |
| CDS                   | `amrfinderplus_element_name`                                               | AMRFinderPlus   | Product description                                                                                                                                                                                         |
...
| CDS                   | `amr_operon_status`                                                        | AMRFinderPlus   | Completeness status of Stx operon                                                                                                                                                                           |
| CDS                   | `dbcan_prot_type`                                                          | run_dbCAN       | Predicted protein function: transporter (TC), transcription factor (TF), signal transduction protein (STP), CAZyme, sulfatase, peptidase                                                                    |
| CDS                   | `dbcan_annotation`                                                         | run_dbCAN       | Predicted protein annotation                                                                                                                                                                                |
...
| CDS                   | `defense_finder_activity`, `defense_finder_type`, `defense_finder_subtype` | DefenseFinder   | Type and subtype of the anti-phage or anti-defense system found                                                                                                                                             |
...
`mettannotator` uses the user-provided TaxId to look up the species name in ENA and compares it against the [list of organisms](https://github.com/evolarjun/amr/wiki/Curated-organisms) available in AMRFinderPlus. If the taxon is present in the curated set within AMRFinderPlus, the tool will be executed using the `--organism` [option](https://github.com/evolarjun/amr/wiki/Running-AMRFinderPlus#--organism-option).

The AMRFinderPlus output folder generated by `mettannotator` includes an `organism.txt` file, which contains the name of the organism used when running AMRFinderPlus.
