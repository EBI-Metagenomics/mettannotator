...
# This script is adapted from MGnify pipelines toolkit:
    ...
    for record in antismash_analysis["records"]:
        ...
        for feature in record["features"]:
            ...
            if iter_cds and feature["type"] == "CDS":
                ...
                start = int(feature["location"].split(":")[0][1:].replace(">", "").replace("<", ""))
                end = int(feature["location"].split(":")[1].split("]")[0].replace(">", "").replace("<", ""))
        ...
        if "antismash.detection.hmm_detection" in record["modules"].keys():
            ...
            for feature in cds_by_protocluster[0][1]:
                locus_tag = feature.get("cds_name")
                if locus_tag:
        ...
        if "antismash.detection.genefunctions" in record["modules"].keys():
            ...
            tool_data = gene_function_tools.get("smcogs")
            if tool_data:
