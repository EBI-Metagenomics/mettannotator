...
def main(reference, target, outfile, species):
            ...
            if len(decision_dict) == 0:
                ...
                if base in decision_dict and len(decision_dict) == 1:
                    ...
                    if alias not in replacements and base not in replacements.values():
                        ...
                        logging.error("Alias {} or gene {} is already in replacements".format(alias, base))
                        sys.exit()
                    ...
                    if not unique:
                        ...
                        stats_dict["unable_to_decide"] = stats_dict.get("unable_to_decide", 0) + 1
                        print("Replacement gene already occurs in the genome, can't replace")
                    else:
                        already_present_in_replacements, reverse = check_gene_presence(decision_dict, replacements, reverse)
                        if (len(dedupl_dict[base]) == len(decision_dict) and
                                all(len(value) == 1 for value in decision_dict.values())):
                            ...
                            if already_present_in_replacements:
                                ...
                                if len(dedupl_dict[base]) - len(decision_dict) > 1:
                                    ...
                                    replacements, reverse, stats_dict = resolve_duplicate(dedupl_dict[base],
                                                                                          decision_dict, replacements,
                                                                                          reverse, stats_dict, base)
    ...
    if made_replacements != len(replacements):
        sys.exit("Made {} replacements but expected {}".format(str(made_replacements), str(len(replacements))))
...
def resolve_duplicate(genes_to_resolve, reference_dict, replacements, reverse, stats_dict, base):
